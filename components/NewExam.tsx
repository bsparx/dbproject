  ...
  return (
    <div className="w-full max-w-xl mx-auto transform transition-all duration-300">
      <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-all duration-300 ${active ? 'ring-2 ring-blue-500 ring-opacity-50' : ''}`}>
        <button
          className="w-full px-6 py-4 bg-gradient-to-r from-blue-500 to-blue-700 text-white font-semibold text-lg hover:from-blue-600 hover:to-blue-800 transition-all duration-300 flex items-center justify-center gap-2"
          onClick={() => setActive(!active)}
        >
          <span>{active ? "Close Form" : "Create a New Mock Exam"}</span>
          <svg
        ...
        <div className={`transition-all duration-300 ease-in-out ${active ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0'} overflow-hidden`}>
          <form action={formAction} className="p-8">
            ...
            />
            <button className="w-full bg-gradient-to-r from-blue-700 to-blue-800 hover:from-blue-800 hover:to-blue-900 text-white font-semibold py-3 px-6 rounded-lg transform transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-md hover:shadow-lg">
              {isPending ? (
